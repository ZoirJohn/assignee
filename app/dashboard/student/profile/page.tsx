...
export default function StudentProfile() {
        ...
        useEffect(() => {
                const getUser = async () => {
                        return (await supabase.auth.getClaims()).data?.claims;
                };
                const fetchProfile = async () => {
                        const userData = await getUser();
                        const { data } = await supabase.from('profiles').select('*').eq('id', userData?.sub).single();
                        if (data) setProfile({ ...data, email: userData?.user_metadata.email, id: userData?.sub });
                };
