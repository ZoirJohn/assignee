...
export default function StudentTabs() {
            ...
            setAssignments((prev) =>
                prev.map((assignment: TAssignment) =>
                    assignment.id === assignmentId
                        ? { ...assignment, status: 'submitted', image_url: URL.createObjectURL(selectedFile) }
                        : assignment
                )
            );
            setSelectedFiles((prev) => {
    ...
    useEffect(() => {
        const fetchAssignments = async () => {
            if (!teacherId) return;
            try {
                const { data: assignments, error } = await supabase.from('assignments').select('*').eq('created_by', teacherId);
                if (error) return;
                ...
                setGrades(
                    updatedAssignments
                        ...
                        .map((assignment) => ({
                            ...
                            grade: assignment.teacher_grade,
                                        ...
                                        {selectedFiles[assignment.id] && (
                                            <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                                                <p className="text-sm text-gray-600 mb-2">
                                                    Selected file: {selectedFiles[assignment.id].name}
                                                </p>
                                                <div
